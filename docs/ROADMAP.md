...
| --- | --- | --- | --- | --- |
| R0 | 현황·릴리스 근거 정리, OPS-01 | 기능표/태그/Unreleased 동기화, CI Go·Aseprite 버전 artifact, draw_pixels upstream 제출 범위 결정 | 등록 50개와 기능표 일치; 버전 증거 보존; 제출 범위 기록 | 문서 초안 작성 완료, CI 기록·범위 결정 예정 |
| R1 | 실행 계약 확립, SAFE-01·GAP-10·OPS-03 | warnings, capability preflight, 오류 분류 및 응답 규칙 설계 | 위험 작업별 경고·기존 클라이언트 호환 검증; 미지원 버전/API를 변경 전에 거부 | 다음 기능 작업: warnings. capability는 R2 전 완료 |
...
R0의 CI 버전 기록은 `[OPS][CHORE] CI 실행 버전 기록`으로 다음 릴리스 전 마무리한다. upstream #19 제출 범위 결정도 R0 잔여다. 이는 다음 기능 작업을 warnings로 지정하는 것과 별개의 운영 잔여 항목이다.
